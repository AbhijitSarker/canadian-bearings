...
/**
 * Punchout login with session ID
 * Authenticates user via sid parameter
 */
export const authPunchoutLogin = async (sid) => {
  try {
    const response = await apiClient.get(`${endpoints.auth.punchoutLogin}?sid=${sid}`);
    ...
    return {
      success: false,
      error: 'Punchout login failed: Invalid response format',
    };
  } catch (error) {
    const errorMessage = formatAuthError(error);
    return {
